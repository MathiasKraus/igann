...
# @njit
# def act(x):
    # return 0.5 * x * (1 + np.tanh(constant * (x + 0.044715 * x ** 3)))
   
# def act(x):
#     return ne.evaluate('(exp(x) - 1) * (x <= 0) + x*(x > 0)')
...
class ELM_Regressor():
    ...
    def predict_single(self, x, i):
        ...
        x_in = np.dot(x_in, np.expand_dims(self.hidden_mat[i,i*self.n_hid:(i+1)*self.n_hid], 0))
        x_in = act(x_in)
